...
from tools.download.pubmed_downloader import download_from_pubmed
        ...
        async with global_semaphore:
            ...
            async with domain_sem:
                ...
                try:
                    ...
                    # 检查是否是PubMed论文
                    if paper.source_db == "pubmed" and (paper.pubmed_download_url or paper.pmcid):
                        logger.info(f"[Download] Using PubMed downloader for PMID {paper.pmid}")
                        result = await asyncio.to_thread(
                            download_from_pubmed,
                            paper=paper,
                            output_dir=download_dir,
                            timeout=PUBMED_DOWNLOAD_TIMEOUT
                        )

                    # 如果PubMed下载失败或不是PubMed论文，使用通用Waterfall策略
                    if not result or not result["success"]:
                        if result:
                            logger.debug(f"[Download] PubMed download failed, falling back to Waterfall")
                        result = await asyncio.to_thread(
                    ...
                    if result["success"]:
                        ...
                        if stats:
                            # 获取URL用于域名统计
                            url = None
                            if result["source"] in ["pmc_pdf", "pmc_xml"]:
                                url = paper.pubmed_download_url or f"PMC:{paper.pmcid}"
                            elif result["source"] == "pdf_url":
                                url = paper.pdf_url
