...
│   ├── __init__.py
│   ├── clarified_intent.py              # ClarifiedIntent（意图澄清输出）
...
│   ├── __init__.py
│   └── grounded_data_v1.json           # grounded_data 的 JSON Schema（与 models/ 同步）
│
├── state/                              # LangGraph State（TypedDict，4个子图各一个文件）
│   ├── __init__.py
│   ├── intent_state.py                 # 子图1: IntentClarificationState
│   ├── retrieval_state.py              # 子图2: RetrievalState
│   ├── extraction_state.py             # 子图3: ExtractionState
│   └── quality_state.py                # 子图4: QualityModuleState
│
├── pipeline/                           # 管道实现
│   ├── __init__.py
│   │
│   ├── intent/                         # 子图1: 意图澄清（3 Agent）
│   │   ├── __init__.py
│   │   ├── graph.py                    # 子图 LangGraph 组装
│   │   └── agents/
│   │       ├── evaluate_intent/
│   │       │   └── __init__.py
│   │       ├── ask_user_guided/
...
│   │           └── __init__.py
│   │
│   ├── extraction/                     # 子图3: 文字与表格提取（5 Agent）
│   │   ├── __init__.py
│   │   ├── graph.py                    # 子图 LangGraph 组装（含 C↔D 重试回路）
│   │   └── agents/
...
│   │           └── __init__.py
│   │
│   └── quality/                        # 子图4: 数据清洗与质检（4 Agent）
│       ├── __init__.py
│       ├── graph.py                    # 子图 LangGraph 组装（含循环控制）
│       └── agents/
│           ├── assessment/
│           │   └── __init__.py
│           ├── normalization/
...
│               └── __init__.py
│
├── tools/                              # 工具函数（按 Agent 分组，具体文件后续填充）
│   ├── __init__.py
│   ├── evaluate_intent/
│   │   └── __init__.py
│   ├── expand_query/
...
│   ├── quality_rules.yaml              # 质量规则（完整性/一致性阈值等）
│   └── schema_mapping.yaml             # 目标 Schema 字段映射规则
│
└── utils/
    ├── __init__.py
    ├── logger.py                       # 统一日志（含 Agent 执行轨迹）
    └── retry.py                        # 重试策略 + 熔断计数器
```
