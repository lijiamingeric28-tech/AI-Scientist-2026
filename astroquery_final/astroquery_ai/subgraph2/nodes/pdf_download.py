...
def download_from_url(url: str, save_path: str, timeout: int = 60, max_size_mb: int = 50) -> dict:
    """
    从 URL 下载 PDF 文件
...
def download_with_waterfall_strategy(
    ...
    for idx, url_info in enumerate(ordered_urls, 1):
        ...
        try:
            result = download_from_url(url, save_path, timeout, max_size_mb)
...
def pdf_download(state: RetrievalState) -> RetrievalState:
    ...
    def download_single(task):
        ...
        # arXiv 有礼貌性频率限制：每次请求间隔 arxiv_delay 秒
        if paper.get("arxiv_id"):
            time.sleep(pdf_config.get('arxiv_delay', 1.0))

        # 使用瀑布式下载
        result = download_with_waterfall_strategy(
            paper, urls, save_dir,
    ...
    # 有效并发取两者较小值：arXiv 直链请求受 arxiv_max_workers 限制
    effective_workers = min(
        pdf_config['max_workers'],
        pdf_config.get('arxiv_max_workers', 3)
    )
