...
def check_retry(state: QualityGraphState, node: str) -> str | None:
    """
    纯 Retry 检查 (V3.3: 读 retry_by_node[node])。
    ...
    if status == "Retry":
        if retries < MAX_RETRIES:
            logger.warning("[Retry] %s (retry %d/%d)", node, retries + 1, MAX_RETRIES)
            return node
        else:
            logger.warning("[Retry] %s max retries (%d) → HumanReview", node, MAX_RETRIES)
            return NODE_HUMAN_REVIEW

    if status == "Failed":
        logger.warning("[Retry] %s Failed → HumanReview", node)
        return NODE_HUMAN_REVIEW
...
def loop_controller_node(state: QualityGraphState) -> dict[str, Any]:
    ...
    if from_conflict:
        # 来源: C→B (Conflict 要求 Normalization 执行修改) → 回 Conflict 重新验证
