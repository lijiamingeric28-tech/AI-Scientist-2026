...
def get_unpaywall_urls(doi: str, email: str, timeout: int = 30) -> List[Dict]:
    ...
    try:
        # M-24: DOI/email 百分号编码——DOI 可含括号/分号/加号，'?'/'&' 原样
        # 发送会破坏 query 结构；email 含 '+' 按 form 解码会变空格
        doi_str = str(doi).strip()
        if not doi_str.startswith("10."):
            logger.warning(f"[Unpaywall] 非法 DOI 格式（非 10. 前缀），跳过: {doi_str[:80]}")
            return urls
        api_url = (f"https://api.unpaywall.org/v2/{quote(doi_str, safe='')}"
