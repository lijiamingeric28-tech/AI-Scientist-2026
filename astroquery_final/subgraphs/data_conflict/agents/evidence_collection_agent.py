...
class AnomalyVerificationAgent:
    ...
    def run(self, state: QualityGraphState) -> dict[str, Any]:
        ...
        # M-06 fix: list() 拷贝隔离 — 原地 append 会污染 classification 共享列表,
        # 重试轮次重复组标注翻倍且 classification 报告被混入 duplicate_observation 条目
        classified_variances = list(classification.get("classified_variances", []))
