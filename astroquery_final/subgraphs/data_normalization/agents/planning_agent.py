...
Return JSON with: tool_name, tool_code (business logic only), confidence (0-1), reasoning."""
        ...
        # C1 fix: dry-run 前必须过与 normalization_agent 一致的 AST 白名单校验
        # (此前 dry-run 路径无校验且 sandbox 裸暴露 __import__ — 完整逃逸入口)
        # H-12 fix: dry-run 同样走硬超时 (exec + 调用), 防死循环挂死规划节点
        from subgraphs.data_normalization.agents.normalization_agent import (
            _safe_import, _validate_code_ast, _run_with_timeout,
            _SANDBOX_MAX_LOG_ENTRIES,
        )
        if not _validate_code_ast(full_code):
        ...
        try:
            ...
            # C1 fix: __import__ 用白名单包装器, 移除 type (与 normalization_agent 一致)
            sandbox = {
                "__builtins__": {
            ...
            if not outcome["ok"]:
                logger.warning("[ToolPlan] Dry-run FAILED: %s", outcome["error"])
                return None
            ...
            if len(result.get("data", [])) != len(test_records):
                ...
                return None
            # H-12 fix: dry-run 输出大小上限 (log 条数)
            if len(result.get("log", [])) > _SANDBOX_MAX_LOG_ENTRIES:
                logger.warning("[ToolPlan] Dry-run: log too large (rejected)")
                return None
