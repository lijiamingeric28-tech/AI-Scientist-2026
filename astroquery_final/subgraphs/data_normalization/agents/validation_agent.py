...
class ValidationAgent:
    def run(self, state: QualityGraphState) -> dict[str, Any]:
        ...
        # L-15 fix: 删除 critical_expected 死代码 (V3.0 重构残留, 计算结果
        # 从未使用) — Normalization 无法补充缺失字段, 不做 schema 完整性检查
        still_missing = []  # V3.0: schema 完整性由 Assessment 负责, Normalization 只修已有字段

        # Format check
        from quality_pipeline.tools._parse_utils import is_numeric
        missing_units = sum(1 for r in records if is_numeric(r.get("field_value")) and not r.get("field_unit"))  # V4 fix: 空串也算缺失
        # L1 fix: 删除死计算 — 旧 missing_prov 强制要求 page (DB 记录恒计入但不参与判定,
        # 误导性残留); provenance 校验统一走 source_utils.provenance_is_complete (DB/paper 分流)
        ...
        try:
            from quality_pipeline.tools.assessment.statistical_conflict import detect_conflicts_statistical
        ...
        try:
            ...
            oor = sum(1 for s in infer_all_fields(
                records, research_domain=ctx.get("research_domain")).values() if s.get("out_of_range"))
