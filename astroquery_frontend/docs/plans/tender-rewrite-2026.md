...
6. **2026-08-24 用户定调：方法章节写设计，不举例子**。P1–P7 等"作品是什么/怎么做"的章节以**设计细节为主体**（架构分层、模块输入-处理-输出、上下文工程机制、防护链条、路由规则），真实查询只在模板**明确要求示例**的章节出现（P8-2 查找示例 / P9-2 解析示例 / P10-2 统一示例 / P13–P17 代表案例）——旧稿被删的原因就是通篇在不合时宜处引用真实查询，这次不重蹈。
7. **所有人工项显式留白**：需要用户提供材料、人工核对/实验的条目一律写 `【待补：…】` 并列入 §7 人工任务清单，不编造、不留空让人以为漏填。
...
## 7. 人工任务清单（用户执行，全部在文档中以【待补：…】标注）

| # | 任务 | 怎么做 | 对应章节 |
|---|---|---|---|
| 1 | 报名材料 | 挑战杯报名作品名称、最终名称、盖章报名表前两页截图 | P1 |
| 2 | 宣传视频链接 | 夸克网盘分享链接（演示视频做好后填） | P1 |
| 3 | 百炼凭证 | 阿里云百炼控制台调用日志 + 计费截图 | P7/P20 |
| 4 | P18 对照组 1：人工整理 | 同一 M45 需求，人工走 SIMBAD→VizieR→论文摘录全流程，记录耗时/准确率/回溯性，与系统结果对照 | P18 |
| 5 | P18 对照组 2：无质量闭环单轮提取 | 可由我代做：用 m45_real_run 的"质量管线输入（final_output.records）vs 最终输出（structured_data）"的差异统计作为单轮提取基线（字段完整率/单位统一率/错误值数），用户只需确认口径 | P18 |
| 6 | 数据人工核对抽样 | 用前端 bbox 溯源图（全屏查看器）抽核 5–10 条记录的数值与原文一致，记录核对状态（正确/待核对/冲突） | P15/P16 |
| 7 | 提交前自检 | 按 P20 清单逐项过一遍（页数、案例闭环、溯源、对照、代码可核验） | P20 |
