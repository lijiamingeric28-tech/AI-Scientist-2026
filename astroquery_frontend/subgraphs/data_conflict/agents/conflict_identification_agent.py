...
class VarianceAggregationAgent:
    ...
    def run(self, state: QualityGraphState) -> dict[str, Any]:
        ...
        # ── V3.0: 读取多源方差分析结果 ──
        # H3 fix: B→C 时一律以 Normalization 复检结果 (conflict_check) 为准 —
        # 复检无冲突由 variance_count=0 表达, 彻底不回落 Assessment 的过期
        # multi_source_variance (否则已修复的单位/异常在下一轮 Conflict 被重新
        # 检出, 修复成果被掩盖, 造成无谓 C→B 轮次)。
        msv = quality.get("multi_source_variance", {})
        conflict_check = norm_validation.get("conflict_check") or {}
        if is_b_to_c and conflict_check:
