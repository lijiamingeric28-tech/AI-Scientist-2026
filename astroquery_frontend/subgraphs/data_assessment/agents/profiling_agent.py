...
class ProfilingAgent:
    ...
    def run(self, state: QualityGraphState) -> dict[str, Any]:
        ...
        # M-04 fix: 统一读 current_data (仅首次回退 input_data) — 与 quality_assessment/
        # decision 一致, E→A 重入时 profile 不再基于 normalization 修改前的过期数据
        ...
        try:
            from quality_pipeline.tools.assessment.semantic_type import infer_all_fields
        ...
        # ── 检测物理不可行的值 (L-21 fix: 改记录级计数 — 原为 (entity,field) 组合
        #    key 数, 与 decision 的记录数分母量纲不一致, 越界过半判定失真) ──
        out_of_range_records: set[str] = set()
        for idx, rec in enumerate(input_data.get("records", [])):
            fn = rec.get("field_name", "")
            et = rec.get("entity_type", "") or ""
            en = rec.get("entity_name", "") or ""
            # 与 infer_all_fields 相同的 entity-aware key 构造
            if et or en:
                ekey = f"{et}:{en}/{fn}" if (et and en) else (f"{en}/{fn}" if en else fn)
            else:
                ekey = fn
            if semantic_types.get(ekey, {}).get("out_of_range"):
                out_of_range_records.add(rec.get("record_id") or f"__rec_{idx}__")
        out_of_range_count = len(out_of_range_records)

        # ── P1: 分布分析 ──
        try:
            from quality_pipeline.tools.assessment.distribution import profile_field_distributions
        ...
        # ── P2: 异常值检测 ──
        try:
            from quality_pipeline.tools.assessment.outlier import detect_all_fields
