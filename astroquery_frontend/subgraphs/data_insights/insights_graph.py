...
from subgraphs.data_insights.agents.recommendation_agent import RecommendationAgent
from subgraphs.data_insights.agents.synthesis_agent import SynthesisAgent
...
def _field_node(state: QualityGraphState) -> dict[str, Any]:
    return _wrap("FieldInsightAgent", _field.run, "insights")(state)


def _rel_node(state: QualityGraphState) -> dict[str, Any]:
    return _wrap("RelationshipAgent", _rel.run, "insights")(state)


def _rec_node(state: QualityGraphState) -> dict[str, Any]:
    return _wrap("RecommendationAgent", _rec.run, "insights")(state)


def _syn_node(state: QualityGraphState) -> dict[str, Any]:
    return _wrap("SynthesisAgent", _syn.run, "insights")(state)
