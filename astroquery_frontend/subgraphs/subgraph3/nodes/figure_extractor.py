...
def figure_extractor(state: ExtractionState) -> ExtractionState:
    ...
    if not tasks:
        ...
        # M-15: 空页面早退路径补发 figure 终止事件——前端子步骤③不再永久 waiting
        from events import emit_progress
        emit_progress(
            query_id, "extraction", "figure", "skipped",
            data={"figures": 0},
        )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            # Web 埋点：图片提取进度（前端卡 3 步骤③）
            # L-08: 循环内只发 running（completed 统一由循环后埋点发，
            # 避免 len(tasks) 为 5 的倍数时同一步骤两条 completed）
            if completed % 5 == 0 or completed == len(tasks):
                from events import emit_progress
                emit_progress(
                    query_id, "extraction", "figure", "running",
                    progress={"completed": completed, "total": len(tasks)},
                )
            if completed % 50 == 0:
                logger.info(f"[FigureExtractor] 进度 {completed}/{len(tasks)}")

    # Web 埋点：图片提取完成摘要（图证数，契约 D6 事件带 count）
    from events import emit_progress
    emit_progress(
        query_id, "extraction", "figure", "completed",
        progress={"completed": len(tasks), "total": len(tasks)},
        data={"figures": len(all_evidence)},
    )
