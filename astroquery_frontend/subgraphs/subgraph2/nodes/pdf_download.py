...
def pdf_download(state: RetrievalState) -> RetrievalState:
    ...
    if not papers_metadata:
        ...
        # M-15: 空数据早退路径补发 paper/download 终止事件——前端子步骤③不再永久 waiting
        emit_progress(
            query_id, "retrieval", "paper/download", "skipped",
            data={"downloaded": 0, "failed": 0},
        )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                # Web 埋点：下载进度（前端卡 2 论文下载进度条，bibcode）
                emit_progress(
                    query_id, "retrieval", "paper/download", "running",
                    progress={
                        "completed": completed,
                        "total": total,
                        "current": paper.get("bibcode", "unknown"),
                    },
                )
    ...
    # Web 埋点：下载完成（契约：只报"已下载 X 篇"数字，前端拼）
    emit_progress(
        query_id, "retrieval", "paper/download", "completed",
        progress={"completed": total, "total": total, "current": None},
        data={"downloaded": len(downloaded_papers), "failed": len(failed_downloads)},
    )
