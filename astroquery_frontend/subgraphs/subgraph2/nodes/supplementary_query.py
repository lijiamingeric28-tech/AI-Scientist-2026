...
def supplementary_query(state: RetrievalState) -> RetrievalState:
    ...
    for _p_idx, paper in enumerate(papers, 1):
        ...
        # Web 埋点：按论文进度（前端卡 2 补充材料进度条，契约 D6-6）
        emit_progress(
            query_id, "retrieval", "supplementary/find", "running",
            progress={"completed": _p_idx - 1, "total": len(papers)},
        )
        ...
        # L-10: 表级验证进度（D6-6）——每张表验证开始时发 substatus，
        # 前端卡 2 显示"正在验证表 J/MNRAS/427/1463…"（仅 data，不覆盖论文级 progress）
        def _emit_substatus(table_id: str) -> None:
            emit_progress(
                query_id, "retrieval", "supplementary/find", "running",
                data={"substatus": f"正在验证表 {table_id}"},
            )

        judgments: dict = {}
        for _tid in real_tables:
            _emit_substatus(_tid)  # L-10: 每表验证前发 substatus（前端"正在验证表 X"）
    ...
    # Web 埋点：查找完成 + 总结（前端卡 2 补充材料，契约 D6-6）
    emit_progress(
        query_id, "retrieval", "supplementary/find", "completed",
        progress={"completed": len(papers), "total": len(papers)},
    )
    emit_progress(
        query_id, "retrieval", "supplementary/summary", "completed",
        data={"tables": len(supplementary_sources), "records": len(supplementary_records)},
    )
