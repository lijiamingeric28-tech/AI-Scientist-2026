...
以上 7 个步骤被分解为四个独立的子图（Sub-Graph），每个子图内部包含若干 Node/Agent：
...
意图澄清子图是整个 LangGraph 工作流的入口阶段。它负责对用户的自然语言科学查询进行意图解析与参数提取，通过动态生成槽位检查清单、引导式追问和最终人工确认，将模糊的自然语言转化为结构化的检索参数集合（`extracted_parameters`），为下游检索子图提供精确的检索目标。本子图内部包含 3 个 Node，其中 Node B 和 Node C 为 HITL（Human-in-the-Loop）交互节点。
...
#### 1.3.1 Node A: Evaluate_Intent_Agent（意图评估与填槽）
...
| **Logic** | 见下方 Stage 拆分。 |
| **Decision** | • **条件 1（全部填满）：** 调用工具 `submit_final_intent` → 设置 `is_clear=True`，流向 Node C。<br>• **条件 2（未填满且 `turns < 3`）：** 调用工具生成带选项的追问 → 流向 Node B。<br>• **条件 3（未填满且 `turns >= 3`）：** 调用工具 `force_submit_best_guess` 强行补齐参数 → 设置 `compromise_flag=True` 及 `is_clear=True`，流向 Node C。 |
...
| **Failure Strategy** | 大模型生成 Schema 失败或参数提取异常时，返回 Retry；连续失败后，设置 `compromise_flag=True`，使用用户原始输入作为降级参数强制放行至 Node C。 |
...
│
├── Stage 3：Slot Completeness Verification
│      检查所有必填项是否已赋值。
│      ├── 全部填满 → is_clear = True。
│      └── 存在空缺 → 根据 turns 判断路径。
│
└── Stage 4：Decision Routing
       ├── 全部填满 → 调用 submit_final_intent → 流向 Node C。
       ├── 未填满 + turns < 3 → 生成引导式追问选项 → 流向 Node B。
       └── 未填满 + turns >= 3 → 调用 force_submit_best_guess →
           compromise_flag = True + is_clear = True → 流向 Node C。
```
...
#### 1.3.2 Node B: Ask_User_Guided（引导式追问 — HITL）

**定位：** LangGraph 的原生挂起点，负责与前端交互，收集缺失线索。**（本节点为简单 HITL 交互节点，不进行内部 Stage 拆分。）**
...
| **Decision** | 唤醒后，**无条件路由回 Node A** 进行下一轮槽位检查。 |
| **Tool Capability** | • `interrupt()` 挂起机制（LangGraph 原生 HITL）• Frontend Form Renderer（向前端渲染追问选项）• Response Parser（解析前端传回的用户补充信息） |
| **Failure Strategy** | 若用户长时间未响应（超时），自动触发 `force_submit_best_guess`，设置 `compromise_flag=True`，直接跳转至 Node C。 |

---

#### 1.3.3 Node C: Confirm_Intent（最终意图确认 — HITL）

**定位：** 状态覆盖（State Overwrite）的核心节点，人类防线的最后一关。**（本节点为简单 HITL 交互节点，不进行内部 Stage 拆分。）**
...
| 流程节点 | 读取的 State | 写入的 State |
|:---|:---|:---|
| **Node A: Evaluate_Intent_Agent** | `original_query`；`query_context`；`chat_history`；`clarification_turns` | `dynamic_task_schema`；`extracted_parameters`；`is_clear`；`compromise_flag` |
| **Node B: Ask_User_Guided** | `extracted_parameters` | `chat_history`；`clarification_turns` |
| **Node C: Confirm_Intent** | `extracted_parameters`；`compromise_flag` | `extracted_parameters`（覆盖） |

### 1.5 工作流控制逻辑

#### 追问循环控制（A ⇄ B 循环）

Node A 与 Node B 之间形成**带计数器的追问闭环**：

1. Node A 评估槽位完整性；
2. 槽位未满且 `clarification_turns < 3` → Node B 发起追问；
3. 用户回答 → Node B 追加 `chat_history`，`clarification_turns + 1` → 无条件路由回 Node A；
4. Node A 重新评估 → 循环直至槽位填满或达到最大追问次数。
...
- 当 `clarification_turns >= 3` 且槽位仍未完全填满时：
  ...
  - 前端在 Node C 的确认表单中高亮警告"部分参数为 AI 推测"；
  - 下游使用该数据时自动降级置信度。

#### 异常处理

- **Node A 大模型生成 Schema / 提取参数失败**：Retry；连续失败后设置 `compromise_flag=True`，使用原始查询作为降级参数强制放行。
- **Node B 用户超时未响应**：自动触发兜底，设置 `compromise_flag=True`，直接跳转 Node C。
- **Node C 用户超时未响应**：自动接受当前 `extracted_parameters`，保持原 `compromise_flag` 值，直接放行至检索子图。
- **所有异常路径均保留完整日志**，供后续回溯。
...
检索子图负责将意图澄清子图输出的结构化意图参数（`extracted_parameters`）翻译为机器友好的多源检索指令，并发调用学术文献库、开放数据库和网页搜索引擎进行资料抓取，再利用引用链扩展突破关键词检索的局限，最终经过全局去重与优先级合并，输出一份干净、无重复的资产清单（`deduplicated_assets`），供下游提取子图使用。本子图内部包含 6 个 Node，其中 Node B1/B2/B3 为并行执行。
...
| `expanded_queries` | Dict | `{}` | 读/写 | Node A 生成的面向不同数据源的具体 API 检索策略（如同义词、布尔逻辑表达式、API 过滤参数） |
...
| `deduplicated_assets` | List[Dict] | `[]` | 写 | **【本子图最终输出产物】** 经 Node D 全局合并、去重、排优先级后的干净资产清单，供下游提取子图使用 |

### 2.3 节点/智能体详细设计

---

#### 2.3.1 Node A: Expand_Query_Agent（检索策略扩展）
...
│
├── Stage 1：Core Entity Extraction
│      读取 intent_params 中的核心实体
│     （如：特定超新星名称、材料牌号、实验条件等）。
│
├── Stage 2：Synonym & Hyponym Expansion
│      结合科学常识生成同义词和上下位词扩展
│     （如别名、不同命名规范、相关术语变体）。
│
└── Stage 3：Source-Specific Query Construction
...
#### 2.3.2 Node B1: Paper_Search（学术文献检索）
...
| **Logic** | 见下方 Stage 拆分。 |
| **Decision** | 固定流向 Node C（Citation_Expansion，提供种子文献）和 Node D（Deduplicate_and_Cluster，直接汇入去重池）。 |
...
Paper Search Logic
│
├── Stage 1：API Query Construction
...
#### 2.3.3 Node B2: Database_Search（开放数据库检索）
...
| **Logic** | 见下方 Stage 拆分。 |
| **Decision** | 固定流向 Node D（Deduplicate_and_Cluster）。 |
...
#### 2.3.4 Node B3: Web_Search（网页材料检索）
...
Web Search Logic
│
├── Stage 1：Search Query Execution
│      将 web_query 提交至搜索引擎 API，
│      限定权威域名白名单（.edu / .gov）。
│
├── Stage 2：Domain-Filtered Fetch
...
#### 2.3.5 Node C: Citation_Expansion（引用链扩展）
...
│      提取 Top N 篇作为种子文献。
│
├── Stage 2：Backward Citation Mining
│      调用 Semantic Scholar / OpenAlex 的 Graph API，
│      提取种子文献的参考文献列表（追溯数据源头）。
│
├── Stage 3：Forward Citation Mining
│      提取引用了种子文献的后续研究
│     （发现基于该数据的新成果）。
│
└── Stage 4：Expansion Deduplication
...
#### 2.3.6 Node D: Deduplicate_and_Cluster（全局去重与聚类）
...
│
├── Stage 1：Exact Identifier Dedup
...
│
├── Stage 3：Priority-Based Merge
│      对同一数据在多个来源中出现的情况进行优先级合并。
│      优先级规则示例：开放数据库直达链接 > 论文 PDF 附件 >
│      网页摘要提及。丢弃高解析成本的冗余路径。
│
└── Stage 4：Final Asset List Generation
...
| 流程节点 | 读取的 State | 写入的 State |
|:---|:---|:---|
| **Node A: Expand_Query_Agent** | `intent_params` | `expanded_queries` |
| **Node B1: Paper_Search** | `expanded_queries` | `raw_papers` |
| **Node B2: Database_Search** | `expanded_queries` | `raw_databases` |
| **Node B3: Web_Search** | `expanded_queries` | `raw_web_supplements` |
| **Node C: Citation_Expansion** | `raw_papers` | `citation_papers` |
| **Node D: Deduplicate_and_Cluster** | `raw_papers`；`raw_databases`；`raw_web_supplements`；`citation_papers` | `deduplicated_assets` |

### 2.5 工作流控制逻辑

#### 并行路由机制

Node A 完成检索策略扩展后，LangGraph 的 Parallel Routing 将 `expanded_queries` 同时分发至 B1、B2、B3 三个检索节点。三者**并发执行、互不阻塞**，最大化召回率。

- Node B1 额外将结果异步发送至 Node C（Citation_Expansion）；
- B1、B2、B3、C 的结果最终全部汇入 Node D（Deduplicate_and_Cluster）；
- Node D 等待全部四个输入就绪后执行（**all-join 屏障**）。

#### 部分失败容忍

- B1 / B2 / B3 中任意一个或多个节点返回空结果或执行失败，**不影响其他并行分支**；
- 对应的数据池为空列表，Node D 正常处理剩余数据；
- 仅当四个输入池**全部为空**时，Node D 返回 Failed，通知上游。

#### 引用链扩展的依赖关系

- Node C 依赖 Node B1 的产出（`raw_papers`）作为种子文献来源；
...
- Node A 大模型生成失败时，使用意图原词作为降级查询，不阻塞检索；
...
提取子图肩负着连接"脏数据源"和下游"完美结构化数据"的重任。它负责将检索子图输出的去重资产列表（`deduplicated_assets`）进行格式标准化、相关性过滤，然后通过大模型进行目标数据提取，再经过独立的忠实度校验环节进行"事不过三"的熔断式纠错，最终装配溯源元数据，输出严格符合"多态溯源协议"的结构化数据（`grounded_data`），供下游数据清洗与质检子图使用。本子图内部包含 5 个 Node，其中 Node C（提取）与 Node D（校验）之间形成带熔断的重试闭环。
...
#### 3.3.1 Node A: Format_Standardization（格式标准化与物理打标）
...
| **Logic** | 见下方 Stage 拆分。 |
| **Decision** | 固定流向 Node B（Relevance_Filtering）。 |
| **Tool Capability** | • PDF Parser（文档解析引擎：提取文字段落、裁剪图片、拆解表格行）• DOM Stripper（网页数据：剥离冗余 DOM，提取核心节点）• ID Tagger（为每个碎片分配唯一 `trace_id`）• BBox Extractor（提取 `[x0, y0, x1, y1]` 及页码）• XPath/JSONPath Extractor（网页/数据库的物理定位提取） |
...
│      选择对应的解析策略。
│
├── Stage 2：Content Decomposition & ID Assignment
│      ├── PDF 文件：调用文档解析引擎，提取文字段落、裁剪图片、
│      │         拆解表格行，为每个碎片分配唯一 trace_id。
│      ├── 网页数据：剥离冗余 DOM，提取核心节点，打上 trace_id。
│      └── 开放数据：为数据库行赋予 trace_id。
│
├── Stage 3：Physical Mapping Construction
...
#### 3.3.2 Node B: Relevance_Filtering（相关性过滤）
...
| **Logic** | 见下方 Stage 拆分。 |
| **Decision** | 固定流向 Node C（Data_Extraction_Agent）。 |
| **Tool Capability** | • BM25 Scorer（词频统计算法）• Lightweight Embedding Model（轻量级向量检索模型）• Top-K Ranker（高潜段落排序与截断） |
| **Failure Strategy** | 若过滤后 `filtered_blocks` 为空（全被剔除），发出警告并保留原始 Top N 块兜底；若算法本身失败，跳过过滤，原样传递全部块至 Node C。 |
...
│
├── Stage 1：Intent Keyword Extraction
...
#### 3.3.3 Node C: Data_Extraction_Agent（目标数据提取）
...
| **Input** | `filtered_blocks`（最终态）；`validation_feedback`（若为重试，则附带 Node D 的纠错指导建议） |
...
| **Logic** | 见下方 Stage 拆分。 |
| **Decision** | 固定流向 Node D（Fidelity_Validation_Agent）。 |
...
| **Failure Strategy** | 提取失败时将 `extraction_attempts` 递增；若 `attempts < 3`，随 `validation_feedback` 打回重试；若 `attempts >= 3`，触发熔断（由 Node D 处理）。 |
...
│      （若为 Node D 打回的重试，则附带 validation_feedback。）
│
...
└── Stage 4：Draft Generation
       ...
       写入 State 供 Node D 校验。
```

---

#### 3.3.4 Node D: Fidelity_Validation_Agent（忠实度校验与熔断）
...
| **Logic** | 见下方 Stage 拆分。 |
| **Decision** | • 完全匹配 → 清除 feedback，放行流向 Node F。• 不匹配且 `attempts < 3` → 生成纠错 feedback，`attempts + 1`，打回 Node C。• 不匹配且 `attempts >= 3` → 触发物理熔断：强行将该字段的 `extracted_value` 设为 `Null`，附加 `is_compromised: True` 错误日志，放行流向 Node F。 |
...
│
└── Stage 4：Final grounded_data Assembly
...
| 流程节点 | 读取的 State | 写入的 State |
|:---|:---|:---|
| **Node A: Format_Standardization** | `raw_assets` | `filtered_blocks`（初始态）；`hard_mapping_dict` |
| **Node B: Relevance_Filtering** | 全局意图参数；`filtered_blocks`（初始态） | `filtered_blocks`（最终态，覆盖） |
| **Node C: Data_Extraction_Agent** | `filtered_blocks`（最终态）；`validation_feedback`（重试时） | `extracted_draft` |
| **Node D: Fidelity_Validation_Agent** | `extracted_draft`；`filtered_blocks`；`extraction_attempts` | `validation_feedback`；`extraction_attempts`（递增） |
| **Node F: Bind_Provenance_Metadata** | `extracted_draft`；`hard_mapping_dict` | `grounded_data` |

### 3.5 工作流控制逻辑

#### 提取-校验重试循环（C ⇄ D）

Node C（提取）与 Node D（校验）之间形成带反馈的重试闭环：

1. Node C 首次提取 → Node D 校验；
2. Node D 发现错漏 → 生成具体纠错 feedback → `extraction_attempts + 1` → 打回 Node C；
3. Node C 接收 feedback，重新提取 → Node D 再次校验；
...
- 当 `extraction_attempts >= 3` 时触发物理熔断：
  ...
  - 放行流向 Node F（不再阻塞整体流程）。
- 熔断后的数据在 Node F 中正常装配，但携带受损标记，供下游和前端识别。

#### 异常处理

- **Node A 单资产解析失败**：跳过该资产，记录错误日志，不阻塞整体。
- **Node B 过滤后为空**：发出警告，保留原始 Top N 块兜底。
- **Node D 校验工具本身失败**（如对照文本块丢失）：直接按熔断处理。
- **Node F hard_mapping_dict 整体不可用**：返回 Failed，进入 Human Review。
- **各 Node 内部工具执行失败**：返回 Retry，超出最大重试次数则进入 Human Review。
...
> **设计说明：** 每一个子 Agent 即一个 Sub-Graph，其内部划分为若干 Stage，每个 Stage 映射为 LangGraph 的一个 Node。具体每个 Stage 需调用哪些工具，仍需在编码阶段进一步确认。
