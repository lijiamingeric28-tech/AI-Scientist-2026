...
"""
    ...
    user_choice = interrupt({
        "type": "final_confirm",
        "title": "确认查询信息",  # Web 结构化（契约 D3-2/D5-2）
        "fields": [
            {"label": "天体名称", "value": str(target_entity)},
            {"label": "查询性质", "value": properties_display},
        ],
    ...
    # M-09: 空输入/未识别输入 → 重发 interrupt 追问 (提示限定 y/m/n);
    # 仅显式 "n"/"取消" 才置 cancelled, 避免空回车被误当作取消
    while user_choice not in ["y", "yes", "确认", "是", "m", "modify", "修改", "n", "取消"]:
        user_choice = interrupt({
            "type": "final_confirm",
            "title": "确认查询信息",  # Web 结构化（M-09 重发携带 error 提示）
            "error": "输入无效，请选择 [确认 (y)] [修改 (m)] 或 [取消 (n)]",
            "fields": [
                {"label": "天体名称", "value": str(target_entity)},
                {"label": "查询性质", "value": properties_display},
            ],
        ...
        if user_choice is None:
            user_choice = ""
        user_choice = str(user_choice).strip().lower()
        logger.debug(f"[final_confirm] 重新输入: {user_choice}")
    ...
    if user_choice in ["y", "yes", "确认", "是"]:
        ...
        # H-10: 补 question 键（Web 澄清事件指引，去分隔符）
        new_query = interrupt({
            "type": "final_confirm_modify",
            "title": "重新输入查询",  # Web 结构化
            "question": "已清空查询信息，请重新输入查询（天体名称 + 性质）",
            "text": "\n↻ 已清空信息，请重新输入查询...\n\n请输入新的查询：",
        })
