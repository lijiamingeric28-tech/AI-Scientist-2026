    ...
    def run(self, state: QualityGraphState) -> dict[str, Any]:
        ...
        if needs_llm:
            try:
                ...
                llm_input = []
                # L-16 fix: 细化 try 粒度 — 单条目构造失败只跳过该条目 (不再整批回退)
                for v in needs_llm:
                    try:
                        ...
                        llm_input.append({
                            ...
                            # L-16 fix: years 元素统一 str() 后排序 (混合 int/str 会抛 TypeError)
                            "years": sorted(str(y) for y in years) if years else None,
                            "max_cohens_d": v.get("max_cohens_d", 0),
                            "context_snippets": snippets[:2],  # 最多2条, 每条300字符
                        })
                    except Exception as e:
                        logger.warning(
                            "[DifferenceClassification] 单条目构造失败, 跳过 LLM: %s (entity=%s, field=%s)",
                            e, v.get("entity_name", ""), v.get("field_name", ""))
                ...
                # 合并 LLM 结果
                # L-16 fix: 细化 try 粒度 — 单条目合并失败只影响该条目 (不入外层 except 整批回退)
                llm_map = {
                    (c.get("entity_type", ""), c.get("entity_name", ""), c.get("field_name", "")): c
                    for c in llm_classifications if isinstance(c, dict)
                }
                for v in needs_llm:
                    try:
                        ...
                        if key in llm_map:
                            ...
                            try:
                                # H-14 fix: LLM confidence 可能为字符串 (prompt_parsing), 强转失败回退 0.5
                                v["cause_confidence_final"] = float(lc.get("confidence", 0.5))
                            except (TypeError, ValueError):
                                v["cause_confidence_final"] = 0.5
                        ...
                        logger.warning(
                            "[DifferenceClassification] 单条目合并失败, 回退 unknown: %s (entity=%s, field=%s)",
                            e, v.get("entity_name", ""), v.get("field_name", ""))
                        v["classified_cause"] = "unknown"
                        v["cause_confidence_final"] = 0.2
                        v["classification_method"] = "fallback"
                        v["classification_reason"] = f"LLM merge error: {e}"
                        classified_variances.append(v)
