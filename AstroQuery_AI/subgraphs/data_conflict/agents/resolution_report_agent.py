    ...
    def run(self, state: QualityGraphState) -> dict[str, Any]:
        ...
        for a in confirmed_anomalies:
            ...
            if atype == "unit_error":
                ...
                anomaly_flags.append({
                    "anomaly_type": atype,
                    # L-25 fix: 保留 field_name/source_ids/severity (cross_id 异常自带, 供人工裁决定位/回填)
                    "field_name": a.get("field_name", ""),
                    "entity_name": a.get("entity_name", ""),
        ...
        report = {
            "metadata": {
                ...
                # L-17 fix: 补写 total_conflicts — human_review 摘要 / export metadata 读取该键 (此前恒 0)
                "total_conflicts": total_variances + total_anomalies,
                "confirmed_anomalies": len(confirmed_anomalies),
                "borderline_anomalies": len(borderline_anomalies),
                "false_positives": len(false_positives),
                "duplicate_groups": len(duplicate_groups),
                # M3 fix: 补写 auto_resolved / human_required — export 层
                # quality_summary 读这两个键 (此前从未写入 → 恒 0/False)
                "auto_resolved": (
                    len(confirmed_anomalies) + len(borderline_anomalies) + len(false_positives)
                    if status in ("Annotated", "No_Variance") else 0
                ),
                "human_required": 1 if route == "HumanReview" else 0,
            },
            ...
            "resolution_plan": {
                ...
                "annotations_to_add": [],
                # H-13 fix: critical 级 flag_for_review 也强制人工介入 (路由 C→E 后必须有待审项)
                # L-25 fix: 保留 field_name/source_ids (cross_id 异常, 供 _extract_pending 回填 source_a)
                "human_review_items": [
                    {
                    ...
                    for f in anomaly_flags
                    if f.get("action") == "human_review"
                    or (f.get("action") == "flag_for_review" and f.get("severity") == "critical")
                ],
            },
            "confidence": {
