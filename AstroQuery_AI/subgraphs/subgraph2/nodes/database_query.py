...
def database_query(state: RetrievalState) -> RetrievalState:
    ...
    try:
        ...
        # M-15: 失败早退路径补发 database/match 终止事件——前端子步骤①不再永久 waiting
        emit_progress(
            query_id, "retrieval", "database/match", "failed",
            data={"matched_catalogs": 0, "total_queries": 0},
        )
    ...
    # Web 埋点：数据匹配完成（前端卡 2 子步骤①，契约 D6-6 data 带数字）
    emit_progress(
        query_id, "retrieval", "database/match", "completed",
        data={"matched_catalogs": len(catalog_ids), "total_queries": total_queries},
    )
    ...
    for cat_key, id_list in catalog_ids.items():
        for id_info in id_list:
            ...
            # Web 埋点：星表进度（前端卡 2 进度条；后端节流由总线/SSE 层控制）
            emit_progress(
                query_id, "retrieval", "database/extract", "running",
                progress={
                    "completed": current_q - 1,
                    "total": total_queries,
                    "current": id_info['catalog_name'],
                },
            )
    ...
    # Web 埋点：提取完成 + 总结（前端卡 2 子步骤②③，契约 D6-6）
    emit_progress(
        query_id, "retrieval", "database/extract", "completed",
        progress={"completed": total_queries, "total": total_queries, "current": None},
    )
    emit_progress(
        query_id, "retrieval", "database/summary", "completed",
        data={
            "success": len(successful_catalogs),
            "failed": len(failed_catalogs),
            "records": len(database_records),
        },
    )
