...
# ── 6 个 Agent ──
from subgraphs.data_export.agents.data_organization_agent import DataOrganizationAgent
from subgraphs.data_export.agents.schema_formatting_agent import SchemaFormattingAgent
from subgraphs.data_export.agents.metadata_generation_agent import MetadataGenerationAgent
from subgraphs.data_export.agents.traceability_construction_agent import TraceabilityConstructionAgent
from subgraphs.data_export.agents.output_validation_agent import OutputValidationAgent
from subgraphs.data_export.agents.export_generation_agent import StructuredExportGenerationAgent
...
_export_en = StructuredExportGenerationAgent()
...
def _data_organization_node(state: QualityGraphState) -> dict[str, Any]:
    return _wrap("DataOrganizationAgent", _organization.run, "export")(state)


def _schema_formatting_node(state: QualityGraphState) -> dict[str, Any]:
    return _wrap("SchemaFormattingAgent", _formatting.run, "export")(state)


def _metadata_generation_node(state: QualityGraphState) -> dict[str, Any]:
    return _wrap("MetadataGenerationAgent", _metadata.run, "export")(state)


def _traceability_node(state: QualityGraphState) -> dict[str, Any]:
    return _wrap("TraceabilityConstructionAgent", _traceability.run, "export")(state)


def _output_validation_node(state: QualityGraphState) -> dict[str, Any]:
    return _wrap("OutputValidationAgent", _validation.run, "export")(state)


def _export_generation_node(state: QualityGraphState) -> dict[str, Any]:
    return _wrap("StructuredExportGenerationAgent", _export_en.run, "export")(state)
