...
class RelationshipAgent:
    ...
    def run(self, state: QualityGraphState) -> dict[str, Any]:
        ...
        from tools.insight.context_builder import build_field_summaries
        summaries = build_field_summaries(state)
        std_map = _build_standard_name_map(ctx)
...
def _build_standard_name_map(ctx) -> dict:
    """从 schema_mapping.yaml 构建 DB 列名 → 标准名映射。"""
    mapping = {}
    try:
        from configs import load_yaml
