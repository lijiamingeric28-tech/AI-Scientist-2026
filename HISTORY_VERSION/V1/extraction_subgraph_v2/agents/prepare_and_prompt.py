"""
Agent 1: Prepare_and_Prompt_Agent
准备与Prompt生成
"""
...
from configs.prompts_extraction import NORMALIZATION_PROMPT, NORMALIZATION_SCHEMA, generate_vlm_prompt
from tools import call_vlm_api

logger = logging.getLogger(__name__)


def prepare_and_prompt_agent(state: ExtractionState) -> ExtractionState:
    """
    Agent 1: 准备与Prompt生成
    ...
    """
    logger.info("=" * 80)
    logger.info("[Agent 1] Prepare_and_Prompt_Agent started")
    logger.info("=" * 80)
    ...
    for paper in papers:
        if paper.get("download_status") == "success" and paper.get("local_path"):
            tasks.append({
            ...
            logger.debug(f"  Task added: {paper.get('id')} | {paper.get('title')[:50]}...")
        else:
            skipped += 1
    ...
    logger.info("[Agent 1] Prepare_and_Prompt_Agent completed")
    logger.info("=" * 80)
...
def normalize_intent_with_llm(intent_params) -> dict:
    ...
    Args:
        intent_params: ClarifiedIntent对象
