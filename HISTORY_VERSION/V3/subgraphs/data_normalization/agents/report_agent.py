...
class ReportAgent:
    def run(self, state: QualityGraphState) -> dict[str, Any]:
        ...
        norm["normalization_status"] = status  # M9 fix: 原 typo 键 normalization_tatus
        ...
        # M9 fix: 真实统计在 mods["per_entity_modifications"] (normalization_agent
        # 按 source 聚合写入); 原读不存在的 modification_log 键 → 恒空 {}。
        per_entity_mods: dict[str, int] = dict(mods.get("per_entity_modifications", {}) or {})
