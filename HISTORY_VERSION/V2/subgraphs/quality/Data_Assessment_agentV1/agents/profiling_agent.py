...
def _field_profiler(data: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for fn, vals in sorted(field_groups.items()):
        from subgraphs.quality.tools._parse_utils import is_numeric, parse_numeric
...
class ProfilingAgent:
    ...
    def run(self, state: QualityGraphState) -> dict[str, Any]:
        ...
        # ── P3: 语义类型推断 ──
        try:
            from subgraphs.quality.tools.assessment.semantic_type import infer_all_fields
            semantic_types = infer_all_fields(input_data.get("records", []))
            tool_count += 1
        except Exception as e:
            semantic_types = {}
        ...
        # ── P1: 分布分析 ──
        try:
            from subgraphs.quality.tools.assessment.distribution import profile_field_distributions
        ...
        # ── P2: 异常值检测 ──
        try:
            from subgraphs.quality.tools.assessment.outlier import detect_all_fields
