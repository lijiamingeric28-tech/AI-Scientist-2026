...
FALLBACK_QUESTION_TEMPLATE = "请补充以下信息：{missing_slots}"
"""当LLM生成追问失败时使用的兜底文本模板"""
...
PROMPT_METADATA = {
    "SCHEMA_GENERATION_PROMPT": {
        ...
        "temperature": 0.1,  # LOW_TEMP
        "timeout": 60,
        "max_retries": 1,
        "input_params": ["query", "domain"],
        "output_format": "JSON"
    },
    "EXTRACTION_PROMPT": {
        "agent": "Agent A",
        "function": "extract_parameters",
        "temperature": 0.5,  # MID_TEMP
        "timeout": 60,
        "max_retries": 1,
        "input_params": ["query", "chat_history", "schema", "current_params"],
        "output_format": "JSON"
    },
    "FORCE_FILL_PROMPT": {
        "agent": "Agent A",
        "function": "force_fill_missing_slots",
        "temperature": 0.5,  # MID_TEMP
        "timeout": 60,
        "max_retries": 1,
        "input_params": ["query", "schema", "current_params", "missing_slots"],
        "output_format": "JSON"
    },
    "CLARIFICATION_PROMPT": {
        "agent": "Agent B",
        "function": "generate_clarification_question",
        "temperature": 0.5,  # MID_TEMP
        "timeout": 60,
        "max_retries": 1,
        "input_params": ["query", "current_params", "schema", "missing_slots"],
        "output_format": "JSON"
    },
    "PARSE_RESPONSE_PROMPT": {
        "agent": "Agent B",
        "function": "parse_user_response",
        "temperature": 0.1,  # LOW_TEMP
        "timeout": 60,
        "max_retries": 1,
        "input_params": ["question", "options", "user_input"],
        "output_format": "TEXT"
    },
    "MODIFICATION_PROMPT": {
        "agent": "Agent C",
        "function": "parse_user_modifications",
        "temperature": 0.1,  # LOW_TEMP
        "timeout": 60,
        "max_retries": 1,
        "input_params": ["current_params", "user_input"],
        "output_format": "JSON"
    }
}
"""Prompt模板的元数据，包含推荐的LLM参数"""
