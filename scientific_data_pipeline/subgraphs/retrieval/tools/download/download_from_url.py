...
from config.constants import DOWNLOAD_TIMEOUT, DOWNLOAD_CONNECTION_POOL_SIZE
...
def download_from_url(url: str, save_path: str, timeout: int = DOWNLOAD_TIMEOUT, retry_count: int = 2) -> dict[str, Any]:
    """
    ...
    # 🚀 性能优化：移除Cookie预热（减少请求次数）
    # Cookie预热对成功率影响不大，但会增加50%的请求时间
    ...
    for attempt in range(retry_count):
        try:
            ...
            # 🚀 性能优化：减少重试等待时间
            if attempt < retry_count - 1:
                wait_time = 2 * (2 ** attempt)  # 2s, 4s（原来是5s, 10s）
