...
合并、去重、过滤、排序论文列表（受ENABLE_PUBMED全局开关控制）
"""

import logging
from subgraphs.retrieval.state import RetrievalState
from subgraphs.retrieval.tools.filter_rank import (
    merge_papers,
    deduplicate_by_id,
    filter_papers,
    calculate_score,
...
from config.constants import OUTPUT_TOP_N, ENABLE_PUBMED
...
def filter_rank_agent(state: RetrievalState) -> RetrievalState:
    ...
    # 🔒 全局开关检查 - 如果PubMed禁用，不合并PubMed论文
    if ENABLE_PUBMED:
        pubmed_papers = state.get("pubmed_papers", [])
        logger.info(f"PubMed papers (ENABLED): {len(pubmed_papers)}")
    else:
        pubmed_papers = []
        logger.info(f"PubMed papers (DISABLED by global config): skipping merge")

    citation_papers = state.get("citation_papers", [])

    print(f"\n>>> 收到论文:")
    print(f"  - Agent B (OpenAlex): {len(papers)} 篇")
    print(f"  - Agent B2 (PubMed): {len(pubmed_papers)} 篇")
    print(f"  - Agent D (引用扩展): {len(citation_papers)} 篇")

    logger.info(f"papers: {len(papers)}, pubmed_papers: {len(pubmed_papers)}, citation_papers: {len(citation_papers)}")

    # 输入校验
    if not papers and not pubmed_papers and not citation_papers:
        print("\n[!]  所有输入论文列表都为空，返回空列表")
        logger.warning("All paper lists are empty")
    ...
    # ===== Node 1: Merge =====
    print(f"\n[步骤 1/5] 合并论文列表")
    logger.info("[Node 1] Merging paper lists (3 sources)")

    merged = merge_papers(papers, citation_papers, pubmed_papers)
