...
from config.constants import ENABLE_PUBMED, PUBMED_EMAIL, PUBMED_API_KEY
from subgraphs.retrieval.tools.pubmed_search import search_pubmed, fetch_paper_details, get_pmc_links, parse_pubmed_metadata
...
def pubmed_search_agent(state: RetrievalState) -> RetrievalState:
    ...
    if not ENABLE_PUBMED:
        ...
        state["pubmed_papers"] = []
        print("\n[LOCKED] PubMed功能已在配置中禁用 (config/constants.py)")
        print("="*80 + "\n")
        return state
